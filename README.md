...
- 컨트롤 배치와 기본적인 속성 설정
- 선택된 항목 추출 기능 구현
- 이벤트 처리와 실시간 UI 업데이트 구현
- radioButton과 checkBox의 차이점과 활용 방법 이해
- 조건문을 활용한 사용자 입력 검증과 피드백 제공
- Tab과 방향키, 스페이스바, Enter키를 활용한 사용자 인터페이스 개선
...
- 구현한 내용 (위 그림 참조)
- Tab을 이용해서 GroupBox 와 버튼 사이를 이동하게 제작하였습니다.
- 방향키를 이용해서 선택 아이템 사이를 이동하게 제작하였습니다.
- 스페이스바를 이용해서 아이템 선택할 수 있게 제작하였습니다.
- Enter키로 버튼을 누를 수 있게 제작하였습니다.
...
- 구현한 내용 (위 그림 참조)
- RadioButton과 CheckBox에서 원하는 항목을 선택하면 그 즉시 정보들이업데이트 되도록 구현하였습니다.
- 예를 들어, 라디오버튼에서 메뉴를 선택하면 총 금액이 바로 업데이트 되고, 체크박스에서 옵션을 선택하면 총 금액이 바로 업데이트 되도록 구현하였습니다.
- 여기서 주문하기 버튼 기능을 함수로 만들어 실시간으로 업데이트 되도록 설계했습니다.